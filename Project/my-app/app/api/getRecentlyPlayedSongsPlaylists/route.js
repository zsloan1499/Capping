...
// Get recently played songs, used for the genre to help with suggested reviews
export async function GET(req) {
  try {
    ...
    const response = await fetch("https://api.spotify.com/v1/me/player/recently-played?limit=3", { // Limit to 5 items
      headers: {
    ...
    // Process only the first 5 songs for genres
    const songsWithGenres = await Promise.all(data.items.slice(0, 3).map(async (item) => { // Enforce a slice to limit to 5
